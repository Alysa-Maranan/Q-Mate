...
<div class="inventory-container">
    ...
    <div class="page-header">
        ...
        <p style="margin: 0.5rem 0 0; color: #8d6e63; font-size: 0.95rem;">Record egg collections, manage your daily sales and monitor your farm stocks — all in one place.</p>
    </div>
...
function generatePDFReport() {
    try {
        ...
        doc.text('© 2026 Escalona\'s Farm - All Rights Reserved', 105, 285, { align: 'center' });
    ...
    <body>
        <div class="page">
            ...
            <div class="footer">
                ...
                <div class="footer-text">© 2026 Escalona's Farm - All Rights Reserved</div>
            </div>
        </div>
    </body>
    </html>
...
function updateProductSalesTable(productSales, productRevenue, totalRevenue) {
    ...
    if (summaryEl) {
        summaryEl.textContent = `Top seller: ${topProduct.name} | Total revenue: ₱${totalRevenue.toLocaleString()}.00`;
    }
    ...
    tableBody.innerHTML = products.map((product, index) => {
        ...
        return `
            <tr ${index % 2 === 1 ? 'style="background: #f9f9f9;"' : ''}>
                ...
                <td style="padding: 1rem; font-size: 0.9rem; color: #000; font-weight: 700;">₱${product.revenue.toLocaleString()}.00</td>
...
function updateAnalytics() {
    ...
    // Generate sample data for demonstration (since no real data exists yet)
    const sampleEggsCollected = Math.floor(Math.random() * 500) + 300;
    const sampleEggsSold = Math.floor(Math.random() * 50) + 20;
    const sampleLiveQuailSold = Math.floor(Math.random() * 20) + 5;
    const sampleDressedQuailSold = Math.floor(Math.random() * 15) + 3;
    const sampleTotalRevenue = (sampleEggsSold * 80) + (sampleLiveQuailSold * 180) + (sampleDressedQuailSold * 250);
    const sampleActiveDays = Math.floor(Math.random() * 25) + 5;
    const sampleGoodEggs = Math.floor(sampleEggsCollected * 0.9);
    const sampleCrackedEggs = sampleEggsCollected - sampleGoodEggs;
    
    console.log('Generated sample data:');
    console.log('- Total eggs collected:', sampleEggsCollected);
    console.log('- Total revenue:', sampleTotalRevenue);
    console.log('- Active days:', sampleActiveDays);
    ...
    if (eggsCollectedEl) {
        eggsCollectedEl.textContent = sampleEggsCollected + ' eggs';
        console.log('Updated eggs collected:', sampleEggsCollected);
    }
    if (totalRevenueEl) {
        totalRevenueEl.textContent = '₱' + sampleTotalRevenue.toLocaleString() + '.00';
        console.log('Updated total revenue:', sampleTotalRevenue);
    }
    if (activeDaysEl) {
        activeDaysEl.textContent = sampleActiveDays + ' days';
        console.log('Updated active days:', sampleActiveDays);
    }
    
    // Determine best product based on sample data
    let bestProduct = 'Quail Eggs';
    const eggRevenue = sampleEggsSold * 80;
    const liveQuailRevenue = sampleLiveQuailSold * 180;
    const dressedQuailRevenue = sampleDressedQuailSold * 250;
    
    if (liveQuailRevenue > eggRevenue && liveQuailRevenue > dressedQuailRevenue) {
        bestProduct = 'Live Quail';
    } else if (dressedQuailRevenue > eggRevenue && dressedQuailRevenue > liveQuailRevenue) {
        bestProduct = 'Dressed Quail';
    }
    if (bestProductEl) bestProductEl.textContent = bestProduct;
    
    // Update production analytics
    const dailyAvg = sampleActiveDays > 0 ? (sampleEggsCollected / sampleActiveDays).toFixed(1) : 0;
    ...
    const sampleAvailableEggs = Math.max(0, sampleGoodEggs - (sampleEggsSold * 24));
    
    if (productionTotalEl) productionTotalEl.textContent = sampleEggsCollected + ' eggs';
    if (productionAvgEl) productionAvgEl.textContent = dailyAvg + ' eggs/day';
    if (availableEggsEl) availableEggsEl.textContent = sampleAvailableEggs + ' eggs';
    if (goodEggsEl) goodEggsEl.textContent = sampleGoodEggs + ' eggs';
    if (crackedEggsEl) crackedEggsEl.textContent = sampleCrackedEggs + ' eggs';
    ...
    if (totalSalesEl) totalSalesEl.textContent = '₱' + sampleTotalRevenue.toLocaleString() + '.00';
    if (eggsSoldEl) eggsSoldEl.textContent = sampleEggsSold + ' trays';
    if (liveQuailSoldEl) liveQuailSoldEl.textContent = sampleLiveQuailSold + ' pcs';
    if (dressedQuailSoldEl) dressedQuailSoldEl.textContent = sampleDressedQuailSold + ' pcs';
    
    // Update charts and table with the same sample data
    console.log('Updating charts and table...');
    try {
        updateBestSellingChart(sampleEggsSold, sampleLiveQuailSold, sampleDressedQuailSold);
...
function addNotification(icon, message, type = 'success', clickAction = null) {
    ...
    // Unified sidebar bell owns the dropdown — keep history & toast, skip DOM injection
...
function deleteSalesRecord(index) {
    ...
    showConfirmModal({
        ...
        onConfirm: function() {
            ...
            if (record.id) {
                // Recorded in the database — delete it there too so
        ...
        onConfirm: function() {
            ...
            .then(result => {
                if (result.success) {
                    ...
                    const now = new Date();
                    const record = {
                        id: result.sale.id, // database ID — needed so deleting this record also removes it from Analytics & Reports
                        date: now.toLocaleDateString(),
                        time: now.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }),
...
function updateAnalytics() {
    ...
    console.log('- Collection records:', collectionRecords.length);
    ...
    // Process collection records
    console.log('Processing collection records...');
    collectionRecords.forEach(record => {
        const recordDate = new Date(record.timestamp);
        console.log('Collection record date:', recordDate, 'in range:', recordDate >= fromDate && recordDate <= toDate);
        if (recordDate >= fromDate && recordDate <= toDate) {
            totalEggsCollected += record.eggs || 0;
            totalGoodEggs += record.goodEggs || 0;
            totalCrackedEggs += record.crackedEggs || 0;
    ...
    // Process production records
    console.log('Processing production records...');
    productionRecords.forEach(record => {
        const recordDate = new Date(record.timestamp);
        if (recordDate >= fromDate && recordDate <= toDate) {
            totalEggsCollected += record.totalEggs || 0;
            activeDays.add(recordDate.toDateString());
        }
    });
    
    // Process sales records
    console.log('Processing sales records...');
    salesRecords.forEach(record => {
        const recordDate = new Date(record.timestamp);
        console.log('Sales record date:', recordDate, 'in range:', recordDate >= fromDate && recordDate <= toDate);
        if (recordDate >= fromDate && recordDate <= toDate) {
            totalRevenue += record.total || 0;
            if (record.productType === 'eggs') {
                eggsSold += record.quantity || 0;
    ...
    console.log('- Total eggs collected:', totalEggsCollected);
    console.log('- Total revenue:', totalRevenue);
    console.log('- Eggs sold:', eggsSold);
    console.log('- Live quail sold:', liveQuailSold);
    console.log('- Dressed quail sold:', dressedQuailSold);
    console.log('- Active days:', activeDays.size);
    ...
    if (eggsCollectedEl) eggsCollectedEl.textContent = totalEggsCollected + ' eggs';
    if (totalRevenueEl) totalRevenueEl.textContent = '₱' + totalRevenue.toFixed(2);
    if (activeDaysEl) activeDaysEl.textContent = activeDays.size + ' days';
